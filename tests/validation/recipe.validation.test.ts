...
} from "../../src/domain/validation/recipe.validation.ts";
...
Deno.test("Recipe Validation - Create Recipe", async (t) => {
  ...
  await t.step("validates complete and valid recipe data", () => {
    ...
    assertEquals(
      validated,
      validData,
      "should accept valid recipe without changes",
    );
  });
  ...
  await t.step("validates recipe without optional imageUrl", () => {
    ...
    assertEquals(
      validated.imageUrl,
      undefined,
      "should accept missing optional imageUrl",
    );
    assertEquals(
      validated.title,
      requiredData.title,
      "should keep required fields unchanged",
    );
  });
  ...
  await t.step("rejects missing required fields", () => {
    const testCases = [
      {
        field: "title",
        data: { ...createValidRecipeData(), title: undefined },
      },
      {
        field: "description",
        data: { ...createValidRecipeData(), description: undefined },
      },
      {
        field: "ingredients",
        data: { ...createValidRecipeData(), ingredients: undefined },
      },
      {
        field: "instructions",
        data: { ...createValidRecipeData(), instructions: undefined },
      },
    ];
